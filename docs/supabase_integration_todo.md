...
## 2. 本地与部署环境配置
- 安装 SDK：在前端运行 `npm install @supabase/supabase-js`
- 在项目根目录创建 `.env.local`，填写 `VITE_SUPABASE_URL`、`VITE_SUPABASE_ANON_KEY`
- 更新 `.gitignore` 排除 `.env.local`
- 在 Vercel 项目面板同步设置上述环境变量

## 3. 封装 Supabase 客户端
- 在 `src/lib/supabaseClient.ts` 内读取环境变量并调用 `createClient`
- 提供 CRUD 助手函数（查询、批量写入、事务/错误处理）
- 输出统一的错误日志与返回结果格式

## 4. 用户身份状态管理
- 新增 Auth Provider：监听 `auth.onAuthStateChange`，维护 `user`、`isLoading`
- 在全局 Context（或 `AppProvider`）暴露 `signIn`、`signOut`、`user`
- 为 UI 添加登录/退出入口，未登录时拦截受限页面或提示登录

## 5. 数据访问层迁移
- 构建 `src/services/dataService.ts`，内部通过 Supabase API 操作数据
- 实现对比赛、配置、阵容、时间段的读取/写入接口
- 保留 `localStorage` 缓存：启动时优先读缓存，再同步 Supabase；离线时降级
- 所有页面调用统一的数据服务层，而非直接访问 `localStorage`

## 6. 默认数据导入流程
- 在用户首次登录且 Supabase 中无记录时，将 `default-data.json` 拆分写入各表
- 实现批量导入函数（按顺序执行或使用事务），导入后写入“已导入”标记
- 处理导入失败的回滚/错误提示

## 7. 页面与状态更新
- 修改页面组件的数据源，从数据服务层获取并处理 Loading/Error 状态
- 更新保存按钮、表单提交逻辑，使用 API 写入并刷新缓存
- 确保跨设备登录能看到一致数据（可选：刷新按钮或轮询）
